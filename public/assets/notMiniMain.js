...
function generateResultsHTML(selectedYear, gradesData) {
  ...
  // Determine final status based on general average and module grades
  let finalStatus = "fail";
  let statusMessage = "";
  const hasModuleBelow8 = ['semester1', 'semester2'].some(semester =>
    semesterResults[semester].some(mod => mod.average < 8)
  );

  if (generalAverage > 10 && !hasModuleBelow8) {
    finalStatus = "pass";
    statusMessage = "🎉 PASSED AUTOMATICALLY (General Average > 10 and no modules below 8)";
  } else {
    finalStatus = "fail";
    statusMessage = generalAverage <= 10
      ? "❌ CONTRÔLE REQUIRED (General Average ≤ 10)"
      : "❌ CONTRÔLE REQUIRED (General Average > 10 but module(s) below 8)";
  }
  ...
  const generalSummary = createEl('summary', {
    html: `<strong>General Average: </strong><span class="${finalStatus}">${generalAverage.toFixed(2)}/20 — ${statusMessage}</span>`
  });
  ...
  // Redo summary for modules & subjects below 8 or low general average
  ...
  if (generalAverage <= 10 || hasModuleBelow8) {
    ...
    if (generalAverage <= 10 && redoModules.length === 0) {
      const p = createEl('p', { text: 'Contrôle required due to General Average ≤ 10, even if all modules are above 8.' });
      redoSection.appendChild(p);
    }
  ...
  window.myUniversityGrades = {
    ...
    summary: `Your general average for ${selectedYear} is ${generalAverage.toFixed(2)}/20 — ${statusMessage}`
  };
